Imports System.Data
Partial Class POSTicketingList
    ...
    Protected Sub gridViewBuses_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles gridViewBuses.Load
        Try
            ...
            If String.IsNullOrEmpty(pos) Then
                Response.Redirect("/POSTicketing.aspx")
            End If
            If String.IsNullOrEmpty(reqDate) Then
                'No Request Date so take for today
                query = "SELECT sl.IDSALE,sl.NAME as 'BusName',sl.DATE_CREAT,sl.FLD103 AS 'Capacity',sl.FLD163 AS 'Hour',sp.IDRELATION,sp.FLD120 as 'CityIN',sp.FLD122 as 'CityOut',sp.PRICE,sp.Total,sp.FLD191 as 'ClientCode'," &
                                        "sp.FLD111 as 'ClientName',sp.DISCOUNT,sp.FLD213 AS 'Subscriptiono',sp.DATE_MOD AS 'CreatedOn'," &
                                            "sp.FLD123 AS 'CreatedBy',FLD118 AS 'Currency'" &
                                            "FROM su.SALES as sl INNER JOIN su.SALES_PROD as sp " &
                                             "ON sl.IDSALE = sp.IDSALE AND DAY(sp.DATE_MOD) = DAY(GETDATE()) AND MONTH(sp.DATE_MOD) = MONTH(GETDATE()) AND YEAR(sp.DATE_MOD) = YEAR(GETDATE())   " &
                                               "WHERE FLD123='" + pos + "'" &
                                             "ORDER BY IDSALE"
            ...
            Else
                ...
                Try
                    ...
                    query = String.Format("SELECT sl.IDSALE,sl.NAME as 'BusName',sl.DATE_CREAT,sl.FLD103 AS 'Capacity',sl.FLD163 AS 'Hour',sp.IDRELATION,sp.FLD120 as 'CityIN',sp.FLD122 as 'CityOut',sp.PRICE,sp.Total,sp.FLD191 as 'ClientCode'," &
                                         "sp.FLD111 as 'ClientName',sp.DISCOUNT,sp.FLD213 AS 'Subscriptiono',sp.DATE_MOD AS 'CreatedOn'," &
                                             "sp.FLD123 AS 'CreatedBy',FLD118 AS 'Currency'" &
                                             "FROM su.SALES as sl INNER JOIN su.SALES_PROD as sp " &
                                              "ON sl.IDSALE = sp.IDSALE AND DAY(sp.DATE_MOD) = DAY(CONVERT(DATETIME,'{0}',103)) AND MONTH(sp.DATE_MOD) = MONTH(CONVERT(DATETIME,'{0}',103)) AND YEAR(sp.DATE_MOD) = YEAR(CONVERT(DATETIME,'{0}',103)) " &
                                                "WHERE FLD123='" + pos + "'" &
            ...
            Try
                ...
                query = "SELECT IDDOCUMENT,NAME, MEMO,FLD220 AS 'MSISDN', FLD221 AS 'IMSI', FLD222 AS 'SimNum',  FLD267 AS 'Place'," &
                                        "FLD269 AS 'TigoSIM',FLD271 AS 'Report',FLD270 AS 'UserMobile' FROM su.DOC"
                Dim dtInfo As DataTable = clsDataLayer.ReturnDataTable(query)
                With dtInfo.Rows(0)
                    Dim posInfo As String = String.Format("POS Name: {0} at {2}, User's Phone: {1}", .Item("Memo").ToString, .Item("UserMobile").ToString, .Item("Place").ToString)
                    ltrPOS.Text = posInfo
                End With
            Catch ex As Exception
                clsDataLayer.LogException(ex)
        ...
        Page.Title = "Per POS Ticketing Details"
    End Sub
End Class
